...
const CONTACTS = {
  ...
  vk: 'https://vk.com/potolkoff03',
  telegram: 'https://t.me/potolkoff2024',
  address: 'г. Улан-Удэ, ул. Примерная, 15 (вход со двора, 2 этаж)',
  email: 'info@potolkoff.ru'
};
...
// Отзывы
const REVIEWS = [
  { name: 'Мария С.', text: 'Делали сатиновые потолки в 3-комнатной. Бригада приехала вовремя, работали аккуратно, убрали за собой. Результат превзошёл ожидания! 🔥', rating: '⭐️⭐️⭐️⭐️⭐️', date: '15 янв 2026', service: 'Натяжные потолки' },
  { name: 'Дмитрий К.', text: 'Ремонт под ключ за 28 дней ровно, как обещали. Все по договору, без доплат и сюрпризов. Очень рекомендую!', rating: '⭐️⭐️⭐️⭐️⭐️', date: '10 янв 2026', service: 'Ремонт под ключ' },
  { name: 'Ольга В.', text: 'Сделали санузел под ключ. Плитка положена идеально, гидроизоляция — на совесть. Спасибо мастерам!', rating: '⭐️⭐️⭐️⭐️⭐️', date: '8 янв 2026', service: 'Санузел' },
  { name: 'Алексей П.', text: 'Заказывали глянцевые потолки. Работа заняла 5 часов, комната преобразилась. Цена соответствует качеству.', rating: '⭐️⭐️⭐️⭐️', date: '3 янв 2026', service: 'Натяжные потолки' }
];
...
const orderWizard = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    if (ctx.message?.text === '🏠 Главное меню') return backToMain(ctx);
  ...
  async (ctx) => {
    ...
    if (ctx.message.text === 'Введу номер вручную') {
      await ctx.reply(
        ...
        {
          parse_mode: 'HTML',
          reply_markup: Markup.keyboard([['❌ Отмена', '🏠 Главное меню']]).oneTime().resize()
        }
      );
      return;
    }
...
// Reply Keyboard (постоянное меню внизу экрана)
function getMainMenuReply() {
  return Markup.keyboard([
    ['✨ Потолки', '🏗 Ремонт'],
    ['📸 Наши работы', '⭐️ Отзывы'],
    ['🎁 Акции', '🧮 Калькулятор'],
    ['📞 Контакты', '📞 Заказать звонок']
  ]).resize();
}

// Inline Keyboard (для навигации внутри разделов)
function getMainMenuInline() {
  return Markup.inlineKeyboard([
    [
      Markup.button.url('📱 Telegram', CONTACTS.telegram),
      Markup.button.url('👥 ВКонтакте', CONTACTS.vk)
    ],
    [
      Markup.button.url('📞 Позвонить', 'tel:' + CONTACTS.phone.replace(/-/g, '')),
      Markup.button.callback('📧 Email', 'info_email')
    ]
  ]);
}
...
// Обработка произвольного текста - всегда показываем меню
bot.on('text', async (ctx) => {
  ...
  if (ctx.from.id.toString() === process.env.ADMIN_ID && ctx.message.reply_to_message) {
    ...
    if (match) {
      ...
      try {
        await ctx.telegram.sendMessage(
          userId,
          '👨‍💼 <b>Сообщение от менеджера Потолкоф:</b> ' + ctx.message.text,
          { parse_mode: 'HTML' }
        );
  ...
  // Если пользователь в сцене - не обрабатываем
  ...
  // Проверяем кнопки reply keyboard
  if (text === '✨ Потолки') {
    await showCeilingsMenu(ctx);
  } else if (text === '🏗 Ремонт') {
    await showRenovationMenu(ctx);
  } else if (text === '📸 Наши работы') {
    await showPortfolio(ctx);
  } else if (text === '⭐️ Отзывы') {
    ...
    await ctx.scene.enter('ORDER_SCENE', {
      interest: 'Заказать звонок',
      type: 'general'
    });
  } else if (text === '🏠 Главное меню') {
    await sendMainMenu(ctx);
  } else {
    // На произвольный текст - показываем меню
    await ctx.reply(
      '🤖 <b>Я бот студии Потолкоф!</b>' + '\n\n' +
...
async function showCeilingsMenu(ctx) {
  await ctx.reply(
...
bot.action(/^view_c_(.+)$/, async (ctx) => {
  ...
  try {
    await ctx.editMessageText(
      ...
      '👥 Выбрали <b>' + item.views + ' клиентов</b> за месяц' + '\n\n' +
      '⚡️ <b>Акция сегодня:</b>' + '\n' +
      ...
      '⏰ До конца акции: <b>' + getPromoTimeLeft() + '</b>',
      {
        ...
        reply_markup: Markup.inlineKeyboard([
          ...
          [
            ...
            Markup.button.callback('🏠 Главное меню', 'back_home')
          ]
        ])
      }
    );
...
async function showRenovationMenu(ctx) {
  await ctx.reply(
...
bot.action(/^view_r_(.+)$/, async (ctx) => {
  ...
  try {
    await ctx.editMessageText(
      ...
      '<b>Что входит:</b>' + '\n' +
      item.features.map(f => f).join('\n') + '\n\n' +
      '👥 <b>' + item.views + ' клиентов</b> заказали в этом месяце' + '\n\n' +
      '⚡️ <b>Акция:</b>' + '\n' +
      '🎁 Дизайн-проект в подарок' + '\n' +
...
async function showPortfolio(ctx) {
  const caption =
    '<b>📸 ПОРТФОЛИО НАШИХ РАБОТ</b>' + '\n\n' +
    '✨ Натяжные потолки — 2000+ объектов' + '\n' +
    '🏗 Ремонт квартир — 350+ объектов' + '\n\n' +
    'Смотрите реальные фото в наших соцсетях 👇';
  ...
  await ctx.reply(caption, {
    ...
    reply_markup: Markup.inlineKeyboard([
      [Markup.button.url('📱 Telegram', CONTACTS.telegram)],
      [Markup.button.url('👥 ВКонтакте', CONTACTS.vk)],
      [Markup.button.url('📹 YouTube', 'https://youtube.com/@potolkoff')],
      [Markup.button.callback('🏠 Главное меню', 'back_home')]
    ])
  });
}
...
// Калькулятор
async function showCalculator(ctx) {
  await ctx.reply(
...
async function showContacts(ctx) {
  await ctx.reply(
    ...
    'Сб: 10:00 - 16:00' + '\n' +
    'Вс: выходной' + '\n\n' +
    '💬 <b>Telegram:</b> ' + CONTACTS.telegram + '\n' +
    ...
    '• Рассрочка 0%',
    {
      ...
      reply_markup: Markup.inlineKeyboard([
        ...
        [Markup.button.url('📱 Telegram', CONTACTS.telegram)],
        [Markup.button.url('👥 ВКонтакте', CONTACTS.vk)],
        [Markup.button.url('📱 WhatsApp', 'https://wa.me/79834208805')],
        [Markup.button.callback('🏠 Главное меню', 'back_home')]
      ])
    }
  );
...
bot.action(['portfolio_ceilings', 'portfolio_renovation'], async (ctx) => {
  try {
    await showPortfolio(ctx);
    await ctx.answerCbQuery();
  } catch (e) {
...
bot.action('info_email', async (ctx) => {
  await ctx.answerCbQuery('📧 Email: ' + CONTACTS.email);
});
...
app.get('/', (req, res) => {
  res.send('🤖 Potolkoff Construction Bot v3.0 (Professional) is Running!');
});
...
const WEBHOOK_URL = process.env.WEBHOOK_URL || (process.env.RAILWAY_PUBLIC_DOMAIN ? 'https://' + process.env.RAILWAY_PUBLIC_DOMAIN + '/webhook' : null);
...
app.listen(PORT, async () => {
  ...
  if (WEBHOOK_URL) {
    ...
    console.log('💡 For local development, use polling mode');
  }
});
